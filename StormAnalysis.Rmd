...
c("type of the weather disturbance", "number of persons injured as a result of the weather disturbance", "number of persons killed as a result of the weather disturbance", "damage to property resulting from the weather disturbance, divided by a power of 10", "character representing the power of 10 to multiply with PROPDMG to get net property damage", "damage to corps as a result of the weather disturbance, divided by a power of 10", "character representing the power of 10 to multiply with CROPDMG to get net property damage"))
...
Needing only the above relevant columns, we extract only those. While we're at it, let's also exclude rows with zero values for INJURIES, FATALITIES, PROPDMG, *and* CROPDMG. Even if they don't contribute to the results, the computer would still need processing resources to add those zeroes.
```{r, extractColumns}
...
This function converts the one-character PROPDMGEXP or CROPDMGEXP to
a factor of ten. It will be used in the next code chunk to create `PropertyDamage` and `CropDamage`.
```{r damageMultiplier}
