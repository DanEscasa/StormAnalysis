...
Remove leading and trailing spaces from `EVTYPE`, create new columns for Property and Crop Damage.
```{r, create-new-cols}
...
```{r, cleanEVTYPE}
# Thanks to <a href="https://www.maurotaraborelli.com/projects/reproducible-research-assignment-2/">Mauro Taraborelli</a> for the grep patterns.
colsOfInterest <- colsOfInterest %>% 
...
In summary:
```{r report-summary}
topInjuries   <- head(allDamage[order(allDamage$INJURIES,decreasing = TRUE), ], 1)
...
topFatalities <- head(allDamage[order(allDamage$FATALITIES,decreasing = TRUE), ], 1)
sprintf("The event that caused the most fatalities is %s, with %s fatalities.", topFatalities$EVTYPE, f_comma(topInjuries$FATALITIES, mark = ","))

topPropDamage <- head(allDamage[order(allDamage$PropertyDamage,decreasing = TRUE), ], 1)
...
topCropDamage <- head(allDamage[order(allDamage$CropDamage,decreasing = TRUE), ], 1)
...
cropPlot <- ggplot(top10Crop, aes(x = EVTYPE, y = CropDamage/1000000)) + 
  geom_bar(stat = "identity", position = "dodge") + 
  labs(x = "Event type", y = "Crop Damage") + 
  ggtitle("Total damage by event type \n(in millions)") + 
  theme(axis.text.x = element_text(angle = 90))

PropPlot <- ggplot(top10Prop, aes(x = EVTYPE, y = PropertyDamage/1000000)) + 
  geom_bar(stat = "identity", position = "dodge") + 
  labs(x = "Event type", y = "Property Damage") + 
  ggtitle("Total damage by event type\n (in millions)") + 
  theme(axis.text.x = element_text(angle = 90))
grid.arrange(injurePlot, fatalPlot, nrow = 1)
grid.arrange(cropPlot, PropPlot, nrow = 1)
```
